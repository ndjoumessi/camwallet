...
- Releases are cut as `vX.Y.Z` git tags + GitHub releases; the three `package.json` versions are bumped together. Pushing to `main` auto-deploys the backend (Railway) and admin (Vercel); the backend `/api/v1/health` echoes the `package.json` version once redeployed. Current version: **v3.5.7**.
